  ...
  if (flag && gameNotOver)
  {
    ...
    if (++moveCounter % 2)
    {
      ...
      case 0:
        ...
        break;
      case 1:
        gtk_button_set_label(statusClick, "PLAYER 1 WON");
        gameNotOver = 0;
        return 0;
      case 2:
        gtk_button_set_label(statusClick, "PLAYER 2 WON");
        gameNotOver = 0;
        return 0;
      }
...
void computerMove()
{
  ...
  for (i = 0; i < 3; i++)
  {
    for (j = 0; j < 3; j++)
    {
      ...
      tempGame[i][j] = 1;
      if (hasAnyoneWon(tempGame) == 1)
        score[i][j] += 100;

      // checks if playing the button causes computer to win
      tempGame[i][j] = 2;
      if (hasAnyoneWon(tempGame) == 2)
        score[i][j] += 200;
  ...
  if (gameDifficulty == 1)
  {
    if (randomNumberLessThan(10) < 4)
    {
      do
      {
        x = randomNumberLessThan(3);
        y = randomNumberLessThan(3);
      } while (pressed[x][y]);
    }
  }
  ...
  case 1:
    ...
    break;
  case 2:
    gtk_button_set_label(statusClick, "COMPUTER WON");
    gameNotOver = 0;
    break;
  }
}
